...
if __name__ == "__main__":
    ...
    cv2.imwrite('template1.jpg',image1)
    pipeline.stop()
    pts_src=np.float32([coordinates[0], coordinates[1], coordinates[2], coordinates[3]])
    im_src = image1
    pointseleceted_dictionary = np.load('pointselected_another2_dictionary.npy', allow_pickle=True).item()
    ...
    for i in range(n):
        ...
        im_dst=cv2.imread("D:\pythonProject3.6/templateimage/another/" + str(j) + ".png")
