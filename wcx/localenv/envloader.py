...
import wcx.localenv.item as item
...
class Env_wrs(object):
    ...
    def loadobj(self, name):
        self.__objpath = os.path.join(self.__this_dir, "obstacles/", name)
    ...
    def addchangableobs(self, nodepath: object, objcm: object, pos: object, rot: object) -> object:
        """
...
# camera location, look from where direction
# camera in simulation environment
def loadEnv_wrs(camp: object = [4000, 0, 1700], lookatpos: object = [0, 0, 1000], searching_cam_start: object = False) -> object:
    # Table width: 120
    # Table long: 1080

    base = pc.World(camp=camp, lookatpos=lookatpos, searching_cam_start=searching_cam_start)
...
def loadUr3e(showrbt: object = False) -> object:
...
def loadUr3ex(rbt: object) -> object:
    rbtx = ur3ex.Ur3EDualUrx(rbt)

    return rbtx


def loadObj(f_name: object, pos: object = (0, 0, 0), rot: object = (0, 0, 0), color: object = (1, 1, 1), transparency: object = 0.5, rot_mat: object = np.zeros((3, 3))) -> object:
    ...
    if np.count_nonzero(rot_mat)==0:
        obj.setRPY(rot[0], rot[1], rot[2])
    else:
        obj.setRPY(rot[0], rot[1], rot[2],rot_mat=rot_mat)
